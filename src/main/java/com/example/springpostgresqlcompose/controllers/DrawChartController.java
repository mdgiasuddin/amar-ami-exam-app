package com.example.springpostgresqlcompose.controllers;

import com.example.springpostgresqlcompose.services.DrawChartService;
import lombok.RequiredArgsConstructor;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import java.io.IOException;

@RestController
@RequestMapping("/api/chart")
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class DrawChartController {

    private final DrawChartService drawChartService;

    @GetMapping("/student/summary")
    public String createStudentBarChart() throws IOException {
        return drawChartService.createStudentBarChart();
    }
}
